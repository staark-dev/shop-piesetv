        ...
        <div class="row">
            <div class="col-md-8">
                <div class="form-group @error('title') has-error @enderror">
                    ...
                    <input class="form-control" name="title" value="{{ $product->title }}" placeholder="Nume Produs" type="text">
                    @error('title')
                ...
                <div class="form-group @error('product_body') has-error @enderror">
                    ...
                    <textarea id="tinymce_basic" name="product_body" aria-hidden="false" style="display: block;">{{ $product->description }}</textarea>
                    @error('product_body')
                    <span class="help-block" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control" name="product_telefon" value="{{ $product->phone }}" placeholder="(+40) 72 xxx x574" type="text">
                </div>
                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        <div id="gallery_show">
                            @foreach(json_decode($product->gallery, true) as $photo)
                            <img class="thumbnail" style="margin: 0px 5px 20px;max-width: 30%;float: left;" src="{{ Storage::disk('public')->url('images/items/' . $photo) }}" />
                            @endforeach
            ...
            <div class="col-md-4">
                <div class="panel panel-warning">
                    ...
                    <div class="panel-body">
                        <img style="margin: 0px auto 20px; max-width: 100%;" class="preview thumbnail" src="{{ Storage::disk('public')->url('images/items/' . $product->image) }}" alt="">
                ...
                <div class="panel panel-danger">
                    ...
                    <div class="panel-body">
                        <select multiple="multiple" name="categories" class="form-control" id="select-1">
                            @foreach ($cat as $item)
                                <option @if($product->categories_id == $item->id)selected @endif value="{{ $item->id }}"><strong>{{ $item->name }}</strong></option>
                            @endforeach
                        </select>
                        ...
                        <select multiple="multiple" name="sub_categories" class="form-control" id="select-1">
                            @foreach ($cat as $item)
                                @if(!empty($item->sub_categories))
                                    ...
                                    <option @if($product->sub_categories_id == $items->id)selected @endif value="{{ $items->id }}">{{ $item->name }}&nbsp;- {{ $items->name }}</option>
                                    @endforeach
                                @endif
                            @endforeach
                        </select>
            ...
            <div class="col-lg-12">
                ...
                <div class="form-group col-md-4 @error('product_price') has-error @enderror">
                    ...
                    <div class="input-group col-md">
                        <input class="form-control" id="append" value="{{ $product->price }}" name="product_price" type="text">
                ...
                <div class="form-group col-md-4 @error('product_stock') has-error @enderror">
                    <label>Stock</label>
                    <div class="input-group col-md">
                        <input class="form-control" name="product_stock" value="{{ $product->stock }}" id="append" type="text">
